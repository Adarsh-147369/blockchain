...
namespace AElf.Contracts.ToDo
{
    ...
    {
        const string author = "1RN22CS125";  // Corrected string declaration
        ...
        public override StringValue CreateTask(TaskInput input)
        {
            ...
            State.Tasks[taskId] = new Task
            {
                ...
                CreatedAt = timestamp,
                UpdatedAt = timestamp,
                Owner = Context.Sender.ToString(),
            };

            State.TaskExistence[taskId] = true;

            // Append task ID to the list of IDs safely
        ...
        public override Empty UpdateTask(TaskUpdateInput input)
        {
            ...
            if (!State.Tasks.ContainsKey(input.TaskId))
            {
                return new Empty(); // Handle case where task doesn't exist
            }
        ...
        public override Empty DeleteTask(StringValue input)
        {
            ...
            if (!State.Tasks.ContainsKey(input.Value))
            {
                return new Empty();
            }
        ...
        public override TaskList ListTasks(StringValue input)
        {
            ...
            foreach (var taskId in taskIds)
            {
                if (State.Tasks.ContainsKey(taskId))
                {
                    ...
                    if (task.Owner == owner) // Filter tasks by owner
                    {
        ...
        public override Task GetTask(StringValue input)
        {
            ...
            return State.Tasks[input.Value];
        }
